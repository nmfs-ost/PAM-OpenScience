...
***Github Organizations:*** Organizations are the 'owners' of shared repositories, discussions, and projects where these things are kept completely separate from each other. For example, NOAA Fishries has a single nmfs-ost Github Org, and each science center has a single Github Org. Best practices suggest minimizing the number of existing Github Organizations.

***Github Teams:*** Github Teams are groups of users of common projects, and Github Teams provides a mechanism for access to collections of repositories. There can also be teams within teams. For example: the PAM Strategic Initiative Team has a number of additional teams within it (one for each PAM-SI Team).
...
::: callout-warning
## NO PII/BII are allowed on Github (including confidential fisheries data, passwords, etc !)
:::

1.  **Identify Team Members.** Ensure that all relevant NOAA personnel are listed as team members. If there are non-NOAA partners who require administrative access to the repository, refrain from transferring those repositories (you can still 'point' to those repositories from the Team Manual).
